export type AppMeta = {
  id: string
  name: string
  route: string
  icon: string
  gradient: string
}

// 主屏幕应用 - 先用可爱emoji，你放好图片后会自动替换
export const GRID_APPS: AppMeta[] = [
  { id: 'wechat', name: 'Chat', route: '/apps/wechat', icon: '/icons/wechat.png', gradient: 'from-pink-100 to-pink-200' },
  { id: 'gameCenter', name: '游戏大厅', route: '/apps/game-center', icon: '/icons/doudizhu.png', gradient: 'from-pink-100 to-pink-200' },
  // 收藏日记（替换淘宝；默认图标在 /public/icons/diary.svg，可在设置里替换）
  { id: 'diaryVault', name: '日记', route: '/apps/diary-vault', icon: '/icons/diary.svg', gradient: 'from-pink-100 to-pink-200' },
  { id: 'x', name: 'X', route: '/apps/x', icon: '/icons/x.svg', gradient: 'from-pink-100 to-pink-200' },
  { id: 'music', name: '音乐', route: '/apps/music', icon: '/icons/music.png', gradient: 'from-pink-100 to-pink-200' },
]

// Dock 栏应用
export const DOCK_APPS: AppMeta[] = [
  { id: 'settings', name: '设置', route: '/apps/settings', icon: '/icons/settings.png', gradient: 'from-pink-100 to-pink-200' },
  // Dock：避免过于突兀的彩色外框，统一使用浅色底
  { id: 'manual', name: '使用手册', route: '/apps/manual', icon: '/icons/manual.svg', gradient: 'from-pink-100 to-pink-200' },
  { id: 'preset', name: '创作工坊', route: '/apps/preset', icon: '/icons/sms.png', gradient: 'from-pink-100 to-pink-200' },
]

export const ALL_APPS: AppMeta[] = [...GRID_APPS, ...DOCK_APPS]
export const getAppById = (id: string) => ALL_APPS.find((app) => app.id === id)
