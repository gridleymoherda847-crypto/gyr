...
const MALE_NAMES = [
  ...
  // ===== 大幅扩充：避免二周目高重名率 =====
  '景行', '承泽', '怀瑾', '知白', '知远', '听澜', '行舟', '临渊',
  '归鸿', '望舒', '照野', '逐月', '栖迟', '青岚', '沧溟', '星河',
  '星尘', '星阑', '星野', '星沉', '疏影', '清晏', '清越', '清辞',
  '清澈', '清朗', '清衡', '清珩', '清和', '清弦', '清歌', '清影',
  '云澈', '云岑', '云岚', '云澜', '云涧', '云鹤', '云阙', '云渡',
  '风临', '风澈', '风玄', '风祈', '风沐', '风岚', '风眠', '风致',
  '霁月', '霁寒', '霁川', '霁野', '霁舟', '霁明', '霁尘', '霁白',
  '寒川', '寒澈', '寒澜', '寒舟', '寒砚', '寒辞', '寒祈', '寒屿',
  '墨白', '墨言', '墨羽', '墨尘', '墨离', '墨遥', '墨行', '墨栖',
  '夜阑', '夜澈', '夜行', '夜祈', '夜澜', '夜寂', '夜岚', '夜离',
  '凌风', '凌川', '凌霁', '凌澈', '凌渊', '凌羽', '凌寒', '凌尘',
  '青衡', '青珩', '青岑', '青尘', '青砚', '青临', '青澈', '青川',
  '玄策', '玄衡', '玄澈', '玄尘', '玄羽', '玄祈', '玄离', '玄烬',
  '白砚', '白泽', '白珩', '白羽', '白澈', '白临', '白翊', '白珣',
  '苏澈', '苏离', '苏砚', '苏珩', '苏临', '苏言', '苏祈', '苏羽',
  '顾言', '顾珩', '顾离', '顾临', '顾川', '顾澈', '顾砚', '顾霁',
  '楚砚', '楚珩', '楚临', '楚言', '楚澈', '楚离', '楚祈', '楚川',
  '沈砚', '沈临', '沈珩', '沈言', '沈离', '沈澈', '沈霁', '沈祈',
  '叶珩', '叶离', '叶临', '叶言', '叶澈', '叶霁', '叶川', '叶羽',
  '林澈', '林珩', '林离', '林临', '林言', '林霁', '林舟', '林砚',
  '江临', '江澈', '江珩', '江离', '江言', '江霁', '江舟', '江屿',
  '温言', '温珩', '温离', '温临', '温澈', '温霁', '温舟', '温岚',
  '蓝澈', '蓝珩', '蓝离', '蓝临', '蓝言', '蓝霁', '蓝舟', '蓝岚',
  '云砚', '云珩', '云离', '云临', '云言', '云霁', '云舟', '云屿',
  '墨珩', '墨临', '墨霁', '墨舟', '墨屿', '墨澜', '墨衡', '墨祈',
  '夜珩', '夜临', '夜言', '夜霁', '夜舟', '夜屿', '夜衡', '夜烬',
  '凌珩', '凌临', '凌言', '凌霁', '凌舟', '凌屿', '凌衡', '凌烬',
  '风珩', '风临', '风言', '风霁', '风舟', '风屿', '风衡', '风烬',
  '霜珩', '霜临', '霜言', '霜霁', '霜舟', '霜屿', '霜衡', '霜烬',
  '澜舟', '澜渊', '澜生', '澜止', '澜声', '澜屿', '澜庭', '澜夜',
  '渊止', '渊衡', '渊临', '渊澈', '渊尘', '渊行', '渊舟', '渊白',
  '砚舟', '砚临', '砚霁', '砚尘', '砚白', '砚澈', '砚衡', '砚离',
  '珩川', '珩舟', '珩临', '珩澈', '珩尘', '珩言', '珩离', '珩霁',
  '祈安', '祈年', '祈明', '祈白', '祈川', '祈舟', '祈临', '祈澈',
  '承安', '承泽', '承渊', '承霁', '承珩', '承临', '承澈', '承言',
  '言川', '言舟', '言临', '言澈', '言霁', '言白', '言尘', '言离',
  '明川', '明舟', '明临', '明澈', '明霁', '明珩', '明衡', '明离',
  '景川', '景舟', '景临', '景澈', '景霁', '景珩', '景衡', '景离',
  '长渊', '长舟', '长临', '长澈', '长霁', '长珩', '长衡', '长离',
  '沉舟', '沉渊', '沉珩', '沉澈', '沉霁', '沉临', '沉言', '沉离',
  '远舟', '远渊', '远珩', '远澈', '远霁', '远临', '远言', '远离',
  '知行', '知止', '知遇', '知望', '知澈', '知霁', '知临', '知珩',
  '怀安', '怀远', '怀川', '怀澈', '怀霁', '怀临', '怀珩', '怀衡',
  '照临', '照澈', '照霁', '照舟', '照渊', '照尘', '照白', '照离',
  '听雪', '听风', '听澜', '听雨', '听月', '听尘', '听白', '听川',
  '行止', '行舟', '行远', '行川', '行澈', '行霁', '行临', '行珩',
  '临风', '临雪', '临川', '临渊', '临澈', '临霁', '临尘', '临白',
  '归尘', '归舟', '归川', '归渊', '归澈', '归霁', '归临', '归白',
  '望川', '望渊', '望舒', '望澈', '望霁', '望临', '望白', '望尘',
  '逐风', '逐月', '逐川', '逐渊', '逐澈', '逐霁', '逐临', '逐白',
]
...
function getEvents(): EventDef[] {
  return [
    ...
    // ===== 散修转机：根骨极佳也会被看中（避免一直散修太枯燥）=====
    {
      id: 'E106_loose_recruit_master',
      title: '散修遇师',
      minAge: 9, maxAge: 60,
      condition: (g) =>
        hasFlag(g, 'is_loose_cultivator') &&
        !hasFlag(g, 'in_sect') &&
        ...
        return [
          {
            ...
            effect: (g2, rng2): GameState => {
              let next: GameState = { ...g2, sect: sect.name }
              next = addFlag(next, 'loose_recruited')
              next = addFlag(next, 'in_sect')
              // 分配师父（温柔但很强）
              let master = createPerson(
                rng2,
                'master',
                {
                  gender: rng2.chance(0.55) ? 'male' : 'female',
                  realm: rng2.chance(0.6) ? 'nascent' : 'core',
                  age: rng2.nextInt(260, 520),
                  favor: rng2.nextInt(18, 35),
                  appearance: '白衣胜雪，不染尘埃',
                  personality: '温柔体贴',
                },
                !next.hasPastLover,
                next.gender
          ...
          },
          {
            ...
            text: '仍想做散修',
            effect: (g2) => {
              let next = addFlag(g2, 'loose_recruited')
              next = { ...next, logs: pushLog(next, '你摇头谢绝。\n散修之路艰难，却自由。\n可那句“别浪费根骨”，像一根刺，留在心里。') }
              return next
            },
          },
        ]
...
export default function LiaoliaoYishengScreen() {
  ...
  const [relTab, setRelTab] = useState<'companions' | 'parents' | 'friends' | 'sect' | 'lovers' | 'deceased'>('sect')
  ...
  const handleMarketBuy = (defId: string) => {
    ...
    if (def.kind === 'item') {
      ...
      next = appendPopup(next, '购买成功', `已获得「${def.name}」x${finalQty}。\n可在「物品」里查看/使用。`)
    } else {
  ...
  const handleBeastWork = (personId: string) => {
    if (!game) return
    const p = game.relations.find(r => r.id === personId)
    if (!p || p.status !== 'alive') return
    if (p.companionKind !== 'weak_beast') return
    if (hasPersonFlag(p, `worked_year_${game.age}`)) {
      const next = appendPopup(game, '今年已打工', '这一年它已经出去打过工了。')
      shouldAutoScroll.current = true
      setGame(next)
      return
    }
    if (game.currentEvent || game.yearFlags.popup) {
      const next = appendPopup(game, '先忙正事', '先处理当前事件/弹窗，再安排它去打工。')
      shouldAutoScroll.current = true
      setGame(next)
      return
    }
    ...
    if (rng.chance(0.72)) {
      ...
      next = appendPopup(next, '打工归来', `「${p.name}」带回灵石 +${moneyGain}。`)
    } else {
  ...
  const groupedRelations = useMemo(() => {
    if (!game) return { companions: [] as Person[], parents: [], friends: [], sect: [], lovers: [] as Person[], deceased: [] as Person[] }
    ...
    const companions = specialCompanions
      ...
      // 置顶：剑灵/神话蛋优先，其次弱妖兽
      .sort((a, b) => {
        const w = (p: Person) => (p.companionKind === 'weak_beast' ? 2 : 0)
        return w(a) - w(b)
      })
    return { companions, parents, friends, sect, lovers, deceased }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-3" onClick={() => setShowRelations(false)}>
          <div className="w-[92%] max-w-[420px] max-h-[75vh] rounded-3xl bg-white p-4 overflow-y-auto custom-scrollbar shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mb-3">
              {([
                { id: 'companions', label: `灵契${groupedRelations.companions.length ? `(${groupedRelations.companions.length})` : ''}` },
            ...
            {game.relations.length === 0 ? (
              ...
              <div className="space-y-2">
                {relTab === 'companions' ? (
                  groupedRelations.companions.length === 0 ? (
