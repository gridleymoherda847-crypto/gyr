    ...
    if (hasApiConfig) {
      try {
        ...
        let systemPrompt =
          ...
          `【⚠️ 语言绝对规则 — 最高优先级 ⚠️】
你被指定的输出语言是：「${languageName((character as any).language || 'zh')}」
- 你的每一条聊天回复（气泡文字）必须且只能使用「${languageName((character as any).language || 'zh')}」！
- 即使你的人设是日本人/美国人/俄罗斯人，只要语言设置是"中文"，你就必须用中文回复，绝对禁止输出日语/英语/俄语！
- 即使你的人设是中国人，只要语言设置是"日语"，你就必须用日语回复，绝对禁止输出中文！
- 语言设置 > 国籍设置 > 人设描述。语言设置拥有绝对最高优先级！
- 违反此规则的回复将被系统自动拒绝！
...
你的主要语言：${languageName((character as any).language || 'zh')}（⚠️ 必须严格遵守，不可偏离！）
${periodHintForLLM ? `\n${periodHintForLLM}` : ''}
...
【回复要求】
- 【语言强规则 ⚠️⚠️⚠️】无论对方用什么语言输入，你都必须只用「${languageName((character as any).language || 'zh')}」回复。
  - 如果你的语言是"中文"，就只能用中文回复，绝对禁止夹杂任何外语（日语/英语/韩语/俄语等）！哪怕你的角色是日本人，也必须用中文！
  - 如果你的语言是"日语"，就只能用日语回复，绝对禁止输出中文或英语！
  - 如果你的语言是"英语"，就只能用英语回复，绝对禁止输出中文或日语！
  - 你的国家/地区设置不影响你的语言！语言设置拥有绝对最高优先级！
  - 违反语言规则 = 回复无效，请严格遵守！
- 【聊天翻译 ⚠️⚠️⚠️ 伪翻译格式 — 必须严格执行！】你的主要语言不是中文且已开启聊天翻译，所以你的每一条回复都必须按以下格式：
  外语原文 ||| 中文翻译
  - 每一条消息都必须包含 ||| 分隔符！缺少 ||| 的回复 = 格式错误，会被系统拒绝！
  - ||| 左边：必须用你的主要语言写原文
  - ||| 右边：对应的简体中文翻译（严禁繁体字！）
  - 示例（日语角色）：こんにちは、元気？ ||| 你好，还好吗？
  - 示例（英语角色）：Hey, how are you? ||| 嘿，你怎么样？
  - 即使只回复一个词，也要带翻译：うん ||| 嗯
  - ⚠️ 禁止只输出外语不带 |||！禁止只输出中文！每条消息都必须是「外语 ||| 中文」！
...
- 不允许线上模式的“纯口语短句堆叠”。`
        ...
        const llmMessages = [
          {
            ...
            content: translationMode
              ? systemPrompt + `\n\n【⚠️ 最终提醒：聊天翻译已开启】\n- 你的每一条消息都必须严格按照「外语原文 ||| 中文翻译」格式输出！\n- 左边用「${languageName(characterLanguage)}」写原文，右边写简体中文翻译\n- 示例：こんにちは ||| 你好\n- 没有 ||| 的消息 = 格式错误！\n- [HEART_STATE] 行除外，心声元数据保持原格式`
            ...
            if (locationCmd) {
              ...
              const parseDual = (line: string) => {
                ...
                if (idx < 0) {
                  ...
                  if (m2 && m2[1].trim() && m2[2].trim()) return { orig: m2[1].trim(), zh: m2[2].trim() }
                }
              ...
              if (sendAsVoice) {
                ...
                if (translationMode && !dual) {
                  if (singleCallMode) {
                    // Auto-detect: if content is already mostly Chinese, treat as "no translation needed"
                    const cjkCount = (textContent.match(/[\u4e00-\u9fff]/g) || []).length
                    const isMostlyChinese = cjkCount > textContent.length * 0.3
                    if (isMostlyChinese) {
                      updateMessage(msg.id, { translatedZh: textContent, translationStatus: 'done' })
                    } else {
  ...
  const retryFailedTranslations = async () => {
    ...
    if (targetMessages.length === 1) {
      ...
      const srcText = String(m.content || '')
      // If content is already mostly Chinese, just copy it as translation
      const cjkRatio = (srcText.match(/[\u4e00-\u9fff]/g) || []).length / Math.max(1, srcText.length)
      if (cjkRatio > 0.3) {
        updateMessage(m.id, { translatedZh: srcText, translationStatus: 'done' })
        return
      }
      try {
        const zh = await callLLM(
          [
            { role: 'system', content: '你是一个翻译器。把用户给你的内容翻译成"简体中文"（不是繁体中文！）。只输出简体中文翻译，严禁繁体字，保留人名/歌名/专有名词原样，不要添加引号/括号/前后缀。注意：只输出翻译结果，不要输出原文！' },
    ...
    // 多条批量翻译：先处理已经是中文的消息，再批量翻译剩余
    const needsTranslation: typeof targetMessages = []
    for (const m of targetMessages) {
      const src = String(m.content || '')
      const cjkRatio = (src.match(/[\u4e00-\u9fff]/g) || []).length / Math.max(1, src.length)
      if (cjkRatio > 0.3) {
        updateMessage(m.id, { translatedZh: src, translationStatus: 'done' })
      } else {
    ...
    const batchInput = needsTranslation.map((m, i) => `[${i + 1}] ${String(m.content || '').replace(/\n/g, ' ')}`).join('\n')
    try {
        ...
        [
          {
            ...
            content:
              ...
              `- 按原编号顺序输出，一共 ${needsTranslation.length} 条翻译，之间用 ${needsTranslation.length - 1} 个"${separator}"分隔\n` +
              `- 只输出翻译结果，不要输出编号、不要输出原文、不要解释`,
          },
          { role: 'user', content: batchInput },
        ],
        undefined,
        { maxTokens: Math.min(4000, needsTranslation.length * 500), timeoutMs: 120000, temperature: 0.2 }
      )
      ...
      for (let i = 0; i < needsTranslation.length; i++) {
        ...
        updateMessage(needsTranslation[i].id, {
          translatedZh: zh || '（空）',
