...
- 【转账】想给用户转钱时，必须且只能用这个精确格式（单独一行）：[转账:金额:备注]
  - ✅ 唯一正确格式：[转账:100:红包] 或 [转账:50:给你买奶茶]
  - ❌ 以下格式全部无效，写了也不会触发转账卡片：[向对方转账100]、【转账：100】、[我转账给你]、[转100给你]、"我给你转钱"
  - 重要：如果你写了错误格式，用户看到的只是普通文字，不会收到黄色转账卡片！
  - 要转就直接写正确格式，不想转就完全不要提"转账"二字
...
【最重要】所有生成的内容必须严格基于下方的"与用户的聊天记录"，不能凭空捏造用户不知道的事情！

角色人设：${character.prompt}

=== 与用户（${selectedPersona?.name || '用户'}）的聊天记录（核心参考） ===
${fullContext}
=== 聊天记录结束 ===
...
【核心原则】
- **只能提及聊天记录中实际发生过的事情**，不能编造新的事件或经历
- 聊天内容应该是对"已发生对话"的侧面反映，比如：
  * 如果用户和角色聊过某个话题，角色可以和朋友分享这个话题
  * 如果用户送过礼物/红包，角色可以和朋友提到收到了礼物
  * 如果有约会/见面的对话，角色可以和朋友说见面的感受
- **禁止**生成用户完全不知道的新剧情、新事件、新约会
